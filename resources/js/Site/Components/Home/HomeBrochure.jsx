...
const HomeBrochure = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("OTP has been sent to your email. Please verify to proceed.");
      ...
      <Grid container spacing={4} alignItems="center">
        <Grid item xs={12} md={6}>
          <BrochureContent className="Brochure-section-data">
            ...
            {isLoggedIn ? (
              <Button variant="contained" color="secondary" onClick={() => navigate("/account/brochures")} sx={{ mt: 2 }}>
                Go to Brochures
              </Button>
            ) : (
              <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3 }}>
                {formLoading ? (
                  ...
                  <>
                    <TextField label="Enter OTP" value={otp} onChange={handleOtpChange} fullWidth required />
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={6}>
                      <TextField label="Name" name="name" value={formData.name} onChange={handleInputChange} fullWidth required sx={{ mb: 2 }} />
                      <TextField label="Email" name="email" type="email" value={formData.email} onChange={handleInputChange} fullWidth required />
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <TextField label="Company" name="company" value={formData.company} onChange={handleInputChange} fullWidth sx={{ mb: 2 }} />
                      <TextField
                        label="Phone"
                        name="phone"
                    ...
                    <Grid item xs={12} md={6}>
                      <TextField select label="Service" name="service" value={formData.service} onChange={handleInputChange} fullWidth required>
                    ...
                    <Grid item xs={12} md={6}>
                      <TextField select label="Referral Source" name="source" value={formData.source} onChange={handleInputChange} fullWidth required>
                    ...
                    <Grid item xs={12}>
                      <TextField label="Message" name="message" value={formData.message} onChange={handleInputChange} fullWidth multiline rows={3} />
                    </Grid>
                    <Grid item xs={12}>
